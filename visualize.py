...
def parse_args():
    ...
    parser.add_argument('--result_dir', type=str,
                        default="./result.json", required=False, help='path to result file')
...
def visualize_tracklet(tracklet_id, tracklet, result, cfg, log):
    pred_bboxes = []
    for item in result:
        pred_bbox = Box.decode(item)
        pred_bboxes.append(pred_bbox)

    gt_bboxes = []
    pcds = []
    for frame in tracklet:
        gt_bbox = frame['3d_bbox'] #bbox
    ...
    for pred_bbox, gt_bbox in zip(pred_bboxes, gt_bboxes):
        overlap = estimateOverlap(
            gt_bbox, pred_bbox, dim=cfg.IoU_space, up_axis=cfg.up_axis)
...
def main():
    ...
    test_dataset = get_dataset(cfg, type='test', split=cfg.test_split)
