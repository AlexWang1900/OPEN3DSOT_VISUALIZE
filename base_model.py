...
class BaseModel(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):#fix by wang for new pl del validation_epoch_end
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log('precision/test', self.prec, on_step=True, on_epoch=True)
        #add by wang 
        self.pred_bboxes.append((batch_idx, result_bbs))
        

        return result_bbs

    def on_test_epoch_end(self): #fix by wang for new pl del test_epoch_end
        self.logger.experiment.add_scalars('metrics/test',
                                           {'success': self.success.compute(),
                                            'precision': self.prec.compute()},
                                           global_step=self.global_step)
        ...
        with open("./result.json", 'w') as f:  #osp.join(self.cfg.work_dir, '
            json.dump(data, f)
